...
import net.mcreator.mcfantastica.procedures.PetrifiedSkeletonOnInitialEntitySpawnProcedure;
import net.mcreator.mcfantastica.item.PetrifiedBoneItem;
import net.mcreator.mcfantastica.MCFantasticaElements;

@MCFantasticaElements.ModElement.Tag
public class PetrifiedSkeletonEntity extends MCFantasticaElements.ModElement {
	public static EntityType entity = null;
	public PetrifiedSkeletonEntity(MCFantasticaElements instance) {
		super(instance, 1);
	...
	public void initElements() {
		entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER).setShouldReceiveVelocityUpdates(true)
				.setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).size(0.6f, 1.8f)).build("petrifiedskeleton")
						.setRegistryName("petrifiedskeleton");
		elements.entities.add(() -> entity);
		elements.items.add(() -> new SpawnEggItem(entity, -1, -1, new Item.Properties().group(ItemGroup.MISC)).setRegistryName("petrifiedskeleton"));
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("mcfantastica:bloodswamp")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
			biome.getSpawns(EntityClassification.MONSTER).add(new Biome.SpawnListEntry(entity, 20, 4, 6));
		}
		...
		RenderingRegistry.registerEntityRenderingHandler(CustomEntity.class, renderManager -> {
			BipedRenderer customRender = new BipedRenderer(renderManager, new BipedModel(), 0.5f) {
				protected ResourceLocation getEntityTexture(Entity entity) {
					return new ResourceLocation("mcfantastica:textures/petrifiedskeleton.png");
				}
			};
	...
	public static class CustomEntity extends MonsterEntity {
		...
		protected void dropSpecialItems(DamageSource source, int looting, boolean recentlyHitIn) {
			...
			this.entityDropItem(new ItemStack(PetrifiedBoneItem.block, (int) (1)));
		}
		...
		public ILivingEntityData onInitialSpawn(IWorld world, DifficultyInstance difficulty, SpawnReason reason, ILivingEntityData livingdata,
			...
			Entity entity = this;
			{
				...
				PetrifiedSkeletonOnInitialEntitySpawnProcedure.executeProcedure($_dependencies);
			}
			return retval;
		}
